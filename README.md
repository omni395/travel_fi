...
На данный момент Travel Fi — это прототип на стадии активной разработки. Реализован базовый архитектурный фундамент, доказывающий техническую жизнеспособность концепции. Прототип не является завершенным продуктом и требует доработки кодовой базы, отладки логики и финальной подготовки архитектуры к Production-запуску.
...
| Контракт | Роль |
|----------|------|
| **Эмитент** (`TravelFiToken`) | Жёсткая эмиссия `MAX_SUPPLY = 1 000 000` (18 decimals). Разово распределяет токены по остальным контрактам, затем **ставится на паузу**. |
...
3. Пользователь получает награду за действие:
   - **без локдейса** — действия, дающие токены сразу для траты (по умолчанию: регистрация и реферальные бонусы, lock=0);
   - **с локдейсом (vesting)** — остальные награды (фото, контент, голоса): запись старше лок-периода (`updated_at + lock_days` уже прошло) считается разблокированной; до этого — заблокировано и отображается как внутренний off-chain баланс.
