...
- 🔴 **Предложения правок (Suggested Edits, консенсус 100м):** прямая правка — только автору точки в окно авторства; юзеры в радиусе 100м создают «предложение правки» (`SuggestedEdit`, pending_review), которое применяется по консенсусу (автор / 2-3 независимых локальных юзера / репутация). Уведомление автору через Noticed. Авто-экспирация по отсутствию ответа автора. См. схему в [`docs/README_ru.md`](docs/README_ru.md:359).
- 🔴 **Quick Toggles (мягкий краудсорсинг):** быстрые бинарные статусы («Работает/Не работает», «Вода закончилась», «Очередь», «Закрыто») — голосование Up/Down через `Vote` с низким порогом из `Setting`; `poi.status` не меняется, только индикатор.
- 🔴 **«Сообщить об ошибке»** для критических данных (координаты/категория/статус) — сигнал модераторам без права прямой правки у обычного юзера.
...
- ✅ **CRUD полей перенесён в форму редактирования категории** (под настройками импорта OSM): на вкладке «Fields» остался только реордер строк (`moveUp`/`moveDown`). Добавление/редактирование/удаление поля делается реактивно внутри edit-формы через `Admin::PoiCategoryFieldsReflex` → `PoiCategoryService` (whodunnit гарантируется) → PaperTrail → `VersionObserverJob` → `PoiCategoryBroadcaster`, который перерисовывает активный контейнер — `[data-poi-category-fields]` (show-вкладка) или `[data-poi-category-fields-edit]` (edit-форма)
- ✅ Динамические поля категорий (создание/обновление/удаление/реордер через `update!` для аудита)
- ✅ **Сохранение формы категории без перезагрузки:** убран `redirect_to` из `Admin::PoiCategoriesReflex#update`; UI обновляется только через Broadcaster
- ✅ **Авто-slug:** slug реактивно заполняется из английского названия (`name[en]`) в форме категории; ручное редактирование slug отключает автозаполнение. Обязательные поля (`name[en]`, `slug`) помечены и валидируются
- ✅ **Модалка поля стандартизирована** (по образцу `Ui::ConfirmDialogComponent`): заголовок + кнопка закрытия в одну строку, прокручиваемое тело (`max-h-[85vh]`), футер с кнопками Отмена/Сохранить (и Удалить в edit-режиме)
...
- ✅ **Аудит «кто/что»:** `whodunnit` гарантируется в `PoiCategoryService.create_field/update_field/destroy_field` (`PaperTrail.request.whodunnit`)
- ✅ **Аудит иконки категории:** загрузка/удаление картинки-маркера логируется через `PaperTrailAuditService.log_category_icon_uploaded/removed` (события `category_icon_uploaded`/`category_icon_removed`, audit-only — не затирают открытую форму). Читаемые лейблы в `Ui::AuditEntryComponent`
...
- ✅ **OSM-маппинг в модалке поля:** `osm_keys` (CSV), `osm_value_map` (JSON), `osm_transform` (дропдаун) — хранятся на `poi_category_fields`, заполняются через `PoiCategoryService.field_params`. Seeds префиллят маппинг для `has_showers`, `fee_amount`, `is_free`, `wheelchair_accessible`, `max_height_meters` и др.
- ✅ **Единая нижняя панель кнопок формы категории:** Cancel/Save и Delete (danger) объединены в единый футер внизу edit-формы (над блоком CRUD полей)
- ✅ **Импорт OSM из локального .pbf файла:** админ загружает региональный срез `.osm.pbf` в диалоге импорта → `Admin::PoiCategoriesController#import_pbf` (multipart) → `OsmPbfImportJob` (SolidQueue) → `OsmPbfImportService` (стриминговый парсинг через `OsmPbfParser`, фильтр по `osm_tags`, metadata через `OsmValueTransformer`, дедуп по `osm_id`). Временный файл удаляется в `ensure` джоба. Прогресс через `OsmImportBroadcaster.pbf_progress` + финальный `call`. Покрыто `spec/services/osm_pbf_import_service_spec.rb`, `spec/jobs/osm_pbf_import_job_spec.rb`, `spec/requests/admin/poi_categories_spec.rb`. Installation - macOS: brew install osmium-tool | Ubuntu: sudo apt-get install osmium-tool
- Вырезает из 3 ГБ файла страны только питьевую воду в мелкий файл на 5 МБ
osmium tags-filter spain-latest.osm.pbf n/amenity=drinking_fountain n/man_made=water_tap -o water.osm.pbf
...
- ✅ **On-chain relay (серверная отправка):** `TokenTransactionService.relay!` + `TokenTransactionRelayJob` (SolidQueue) — подпись EIP-155 (RLP/ECDSA в [`Crypto::Ethereum`](lib/crypto/ethereum.rb:1)) приватным ключом оператора (`OPERATOR_PRIVATE_KEY`, `.env`) → `eth_sendRawTransaction` → `transfer(address,uint256)` на **reward pool-контракт** (`REWARDS_CONTRACT_ADDRESS`) — токены берутся из баланса pool (НЕ mint), газ спонсирует оператор (gasless для юзера). **`relay!` устойчив к любым исключениям (в т.ч. `Exception` — WebMock в тестах): помечает `failed`, job не роняет worker.** Результат `build_signed_transaction` (`{ raw:, tx_hash: }`) разворачивается в `relay!` → реальный `eth_sendRawTransaction` (фикс `NameError` «undefined local variable 'tx_hash'» — транзакции раньше вообще не уходили в сеть). После отправки — **верификация receipt** (`wait_for_receipt`, poll `eth_getTransactionReceipt`): `confirmed`/`claimed=true` ставится ТОЛЬКО при `status == 0x1` (Success); revert (`0x0`) или таймаут → `failed` (транзакция не засчитывается, relay честно не проходит). Покрыто unit-спекой и opt-in интеграцией на реальный Base Sepolia (`RUN_BLOCKCHAIN_INTEGRATION=1`).
...
- ✅ **Тест-инфраструктура / флаки (01.09.2026)** — **471 examples, 8 pending**. Два полных прогона: **21 → 13** падений; все оставшиеся — каскад headful-Selenium (`page.reset_session!` / `close_window` / `Element did not appear` после ~10 подряд идущих system-примеров — окна перестают сбрасываться), изолированно каждый проходит. **Исправлены реальные (не-Selenium) падения**: (1) добавлен `rspec-retry` (API 0.6.x — `default_retry_count` + `exceptions_to_retry`; ретрай только на Selenium/Capybara-инфра-ошибках, не на логических); (2) 6 тестов с фикстурами переведены с `fixture_file_upload` на `Rack::Test::UploadedFile` с явным абсолютным путём (`poi_categories_spec`, `poi_category_service_spec`); (3) спек дедупликации `OsmPbfImportService` приведён к реальной семантике `update_existing_poi` — `:created`, без дубля; (4) мгновенный `reload` → polling по БД в тестах удаления фото (`pois_spec:233`, `poi_spec:390`); (5) `voting_spec:119` → `pending` (локация недоступна); (6) `poi_categories_spec`: не-админ ожидает `:found` (302 — редирект `require_admin_or_moderator!`) вместо `:forbidden`. Selenium-флаки помечены `:flaky` (`pois_spec`, `users_spec`, `gamification_spec`, `sidebar_spec`, `user_profile_edit_spec`, `poi_spec`). **Правило**: после любого изменения фронтенда (JS-контроллер/CSS/шаблон ViewComponent) прогонять system-тесты под `RAILS_ENV=test bin/rails assets:precompile` — иначе браузер получает устаревший бандл и «пустые» падения. **Известно**: каскад сброса окон headful-Selenium в полном прогоне — нестабильность окружения, не логика приложения.
