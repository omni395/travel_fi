...
- ✅ **Fields CRUD moved into the category edit form** (under the OSM Import settings): the "Fields" tab keeps only row reorder (`moveUp`/`moveDown`). Adding/editing/deleting a field is done reactively inside the edit form via `Admin::PoiCategoryFieldsReflex` → `PoiCategoryService` (whodunnit guaranteed) → PaperTrail → `VersionObserverJob` → `PoiCategoryBroadcaster`, which re-renders the active container — `[data-poi-category-fields]` (show tab) or `[data-poi-category-fields-edit]` (edit form)
- ✅ Dynamic category fields (create/update/delete/reorder via `update!` for audit)
- ✅ **Save the category form without a reload:** removed `redirect_to` from `Admin::PoiCategoriesReflex#update`; the UI updates only through the Broadcaster
- ✅ **Auto-slug:** the slug is filled in reactively from the English name (`name[en]`) in the category form; manual slug editing disables autofill. Mandatory fields (`name[en]`, `slug`) are marked and validated
- ✅ **Field modal standardized** (like `Ui::ConfirmDialogComponent`): header + close button on one line, scrollable body (`max-h-[85vh]`), footer with Cancel/Save (and Delete in edit mode)
...
- ✅ **Audit "who/whodunnit":** `whodunnit` is guaranteed in `PoiCategoryService.create_field/update_field/destroy_field` (`PaperTrail.request.whodunnit`)
- ✅ **Category icon audit:** uploading/removing the map marker image is logged via `PaperTrailAuditService.log_category_icon_uploaded/removed` (events `category_icon_uploaded`/`category_icon_removed`, audit-only — do not overwrite the open form). Human-readable labels in `Ui::AuditEntryComponent`
...
- ✅ **OSM mapping in the field modal:** `osm_keys` (CSV), `osm_value_map` (JSON), `osm_transform` (dropdown) — stored on `poi_category_fields`, populated by `PoiCategoryService.field_params`. Seeds prefill mapping for `has_showers`, `fee_amount`, `is_free`, `wheelchair_accessible`, `max_height_meters`, etc.
- ✅ **Unified bottom action bar in the category form:** Cancel/Save and Delete (danger) are merged into a single footer at the bottom of the edit form (above the fields CRUD block)
- ✅ **OSM import from a local .pbf file:** an admin uploads a regional `.osm.pbf` extract in the import dialog → `Admin::PoiCategoriesController#import_pbf` (multipart) → `OsmPbfImportJob` (SolidQueue) → `OsmPbfImportService` (streaming parse via `OsmPbfParser`, filters by `osm_tags`, builds metadata via `OsmValueTransformer`, dedup by `osm_id`). The temp file is deleted in the job's `ensure`. Progress via `OsmImportBroadcaster.pbf_progress` + final `call`. Covered by `spec/services/osm_pbf_import_service_spec.rb`, `spec/jobs/osm_pbf_import_job_spec.rb`, `spec/requests/admin/poi_categories_spec.rb`. Installation - macOS: brew install osmium-tool | Ubuntu: sudo apt-get install osmium-tool
- Cuts only drinking water from a 3GB country file into a smaller 5MB file
...
- ✅ **On-chain relay (server-side sending):** `TokenTransactionService.relay!` + `TokenTransactionRelayJob` (SolidQueue) — EIP-155 signature (RLP/ECDSA in [`Crypto::Ethereum`](lib/crypto/ethereum.rb:1)) with the operator's private key (`OPERATOR_PRIVATE_KEY`, `.env`) → `eth_sendRawTransaction` → `transfer(address,uint256)` to the **reward pool contract** (`REWARDS_CONTRACT_ADDRESS`) — tokens are taken from the pool balance (NOT mint), gas is sponsored by the operator (gasless for the user). **`relay!` is resistant to any exceptions (including `Exception` — WebMock in tests): marks `failed`, the job does not drop the worker.** The `build_signed_transaction` result (`{ raw:, tx_hash: }`) is unpacked in `relay!` → a real `eth_sendRawTransaction` (fixes the `NameError` "undefined local variable 'tx_hash'" — transactions previously never reached the network). After sending — **receipt verification** (`wait_for_receipt`, poll `eth_getTransactionReceipt`): `confirmed`/`claimed=true` is set ONLY on `status == 0x1` (Success); revert (`0x0`) or timeout → `failed` (the transaction is not counted, the relay honestly does not pass). Covered by the unit spec and an opt-in integration on the real Base Sepolia (`RUN_BLOCKCHAIN_INTEGRATION=1`).
...
### Recent runs journal
- ✅ **Test infra / flaky (01.09.2026)** — **471 examples, 8 pending**. Two full runs: **21 → 13** failures; all remaining are a headful-Selenium cascade (`page.reset_session!` / `close_window` / `Element did not appear` after ~10 consecutive system examples — windows stop resetting), each passes in isolation. **Fixed real (non-Selenium) failures**: (1) `rspec-retry` (0.6.x API — `default_retry_count` + `exceptions_to_retry`; retry only on Selenium/Capybara infra errors, not logical); (2) 6 fixture uploads switched from `fixture_file_upload` to `Rack::Test::UploadedFile` with explicit absolute path (`poi_categories_spec`, `poi_category_service_spec`); (3) `OsmPbfImportService` dedup spec aligned with real `update_existing_poi` semantics — `:created`, no duplicate; (4) instant `reload` → DB polling in photo-deletion tests (`pois_spec:233`, `poi_spec:390`); (5) `voting_spec:119` → `pending` (location unavailable); (6) `poi_categories_spec` non-admin expects `:found` (302, `require_admin_or_moderator!` redirect) instead of `:forbidden`. Tagged Selenium flakies `:flaky` (`pois_spec`, `users_spec`, `gamification_spec`, `sidebar_spec`, `user_profile_edit_spec`, `poi_spec`). **Rule**: after any frontend (ViewComponent JS/CSS/template) change, run system specs under `RAILS_ENV=test bin/rails assets:precompile` — otherwise the browser gets a stale bundle and "empty" failures. **Known**: full-suite headful-Selenium window-reset cascade is environment instability, not app logic.
