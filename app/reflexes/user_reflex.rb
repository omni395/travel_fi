...
class UserReflex < ApplicationReflex
  ...
  def update_profile(name)
    ...
    # Получаем файл аватара из DOM элемента (если был выбран)
    avatar_input = element.querySelector("[name=avatar]")
    if avatar_input&.files&.first
      update_params[:avatar] = avatar_input.files.first
    end
    ...
    # Model.after_commit вызовет UserBroadcaster
    # Broadcaster отправит CableReady команды для обновления компонента в браузере
  rescue Pundit::NotAuthorizedError => e
    # Если пользователь не авторизован
    send_error("You are not authorized to update this profile")
    ...
    send_error("An error occurred while updating your profile")
  end
  ...
  private def send_success(message = nil)
    cable_ready[current_user.to_gid_param].dispatch_event(
      name: "usersSuccess",
      detail: { message: message || "Profile updated successfully!" }
    )
    broadcast
  end
end